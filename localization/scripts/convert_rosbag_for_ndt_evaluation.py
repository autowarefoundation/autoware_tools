...
def parse_args():
    ...
    parser.add_argument('--reference_topic_name', type=str,
                        default='/localization/kinematic_state',
                        choices=['/localization/kinematic_state', '/awsim/ground_truth/localization/kinematic_state'])
...
def check_rosbag(duration: float, topic_name_to_msg_list: dict) -> None:
    ...
    assert df["enough_fps"].all(), \
        f"NG! FPS is not enough in {df[df['enough_fps'] == False]['topic'].values}"

    # Either /sensing/gnss/pose_with_covariance or /initialpose must be included.
    assert len(topic_name_to_msg_list["/sensing/gnss/pose_with_covariance"]) >= 1 or len(topic_name_to_msg_list["/initialpose"]) >= 1, \
    ...
    for msg in topic_name_to_msg_list["/localization/kinematic_state"]:
        ...
        ok = twist.linear.x < 0.1 and twist.linear.y < 0.1 and twist.linear.z < 0.1 \
            and twist.angular.x < 0.1 and twist.angular.y < 0.1 and twist.angular.z < 0.1
        if not ok:
...
if __name__ == "__main__":
    ...
    storage_options = rosbag2_py.StorageOptions(
        uri=str(filtered_rosbag_path), storage_id='sqlite3')
