    ...
    def load_rosbag_data(self, rosbag2_dir_list):
        ...
        for rosbag2_dir in rosbag2_dir_list:
            ...
            if load_acc_topic and load_kinematic_topic:
                ...
                acceleration, kinematic_state = db3converter.read_msg('/localization/acceleration'),  db3converter.read_msg('/localization/kinematic_state')
                if acceleration is None or kinematic_state is None:
                    continue

                previous_acc, previous_vel, previous_ang_vel_z  = acceleration.accel.accel.linear.x, kinematic_state.twist.twist.linear.x, kinematic_state.twist.twist.angular.z
                previous_acc_time, previous_kinematic_time = acceleration.header.stamp.sec + 1e-9 * acceleration.header.stamp.nanosec, kinematic_state.header.stamp.sec + 1e-9 * kinematic_state.header.stamp.nanosec
                current_time = max([previous_acc_time, previous_kinematic_time])

                acceleration, kinematic_state = db3converter.read_msg('/localization/acceleration'),  db3converter.read_msg('/localization/kinematic_state')
                if acceleration is None or kinematic_state is None:
                    break
                current_acc_time, current_kinematic_time = acceleration.header.stamp.sec + 1e-9 * acceleration.header.stamp.nanosec, kinematic_state.header.stamp.sec + 1e-9 * kinematic_state.header.stamp.nanosec
                ...
                while True:
                    ...
                    while current_time > current_acc_time:
                        ...
                        if acceleration is None:
                            break
                        current_acc_time = acceleration.header.stamp.sec + 1e-9 * acceleration.header.stamp.nanosec

                    if acceleration is None:
                            break
                    acc = (current_acc_time - current_time) * acceleration.accel.accel.linear.x + (current_time - previous_acc_time) * previous_acc
                    ...
                    while current_time > current_kinematic_time:
                        previous_vel, previous_ang_vel_z = kinematic_state.twist.twist.linear.x, kinematic_state.twist.twist.angular.z
                        ...
                        if kinematic_state is None:
                            break
                        current_kinematic_time = kinematic_state.header.stamp.sec + 1e-9 * kinematic_state.header.stamp.nanosec

                    if kinematic_state is None:
                            break
                    vel = (current_kinematic_time - current_time) * kinematic_state.twist.twist.linear.x + (current_time - previous_kinematic_time) * previous_vel
                    vel /= (current_kinematic_time - previous_kinematic_time)

                    ang_vel_z = (current_kinematic_time - current_time) * kinematic_state.twist.twist.angular.z + (current_time - previous_kinematic_time) * previous_ang_vel_z
                    ...
                    steer = arctan2(
                        wheel_base * ang_vel_z, kinematic_state.twist.twist.linear.x
                    )
