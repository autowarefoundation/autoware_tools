...
from .tree import TreeBase, SummarizedProcessingTimeTree
...
def print_trees(
    ...
    topic_showing = (
        abbreviate_topic(topic_name) if len(topic_name) > width else topic_name
    )[: width - 2]
    stdscr.addstr(1, 0, topic_showing, curses.color_pair(1))

    if summarize:
        trees = [
            SummarizedProcessingTimeTree.from_processing_time_tree(tree)
            for tree in trees
        ]

    tree_lines = list(
        chain.from_iterable(tree.to_lines(show_comment) + [""] for tree in trees)
    )
