...
@dataclass
class GraphNode:
    """Represents a ROS2 node in the graph with its publishers and subscribers"""
...
class TrajectoryNodeGraph:
    """Library class for analyzing ROS2 trajectory node graphs"""
    
    def __init__(self, node=None):
        """
        Initialize TrajectoryNodeGraph
        Args:
            node: ROS2 Node instance (optional, will create one if not provided)
        """
    ...
    def analyze(self):
        """
        Analyze the ROS2 graph and collect data
        Returns:
            dict: Analysis results containing graph, topics, and connections
        """
        if not self.node:
            raise RuntimeError("No ROS2 node available for analysis")
        
        # Build the graph
        self.graph = self.build_graph()
        
        # Analyze connections from behavior_path_planner
        start_node = "/planning/scenario_planning/lane_driving/behavior_planning/behavior_path_planner"
    ...
    def filter_graph_with_active_topics(self, yaml_data, active_topics):
        ...
        filtered_result['all_topics'] = sorted(list(all_topics))
        filtered_result['all_topics_with_types'] = [(t, msg_type) for t, msg_type in sorted(all_topics_with_types.items())]
    ...
    def _trace_connections_and_collect(self, graph: Dict[str, GraphNode], node_name: str, visited: Set[str], depth: int):
    ...
    def _collect_connection_topics(self, graph: Dict[str, GraphNode], node_name: str, visited: Set[str]):
    ...
    def _collect_nodes_in_order(self, graph: Dict[str, GraphNode], node_name: str, visited: Set[str]):
        """Collect nodes in connection order"""
        if node_name in visited or node_name not in graph:
            return
...
class TrajectoryNodeGraphRunner(Node):
    """ROS2 Node for running TrajectoryNodeGraph analysis"""
    ...
    def display_filtered_nodes(self):
        ...
        print("\n=== ROS2 Nodes under /planning/ namespace with Publishers ===")
    ...
    def show_node_publishers(self, node_name):
        ...
        try:
            ...
            for topic_name, topic_types in topic_info_list:
                has_target_type = any(
                    msg_type in self.analyzer.target_msg_types 
                ...
                for pub_info in pub_info_list:
                    full_node_name = f"{pub_info.node_namespace}{'/' if not pub_info.node_namespace.endswith('/') else ''}{pub_info.node_name}"
                    if full_node_name == node_name:
                        filtered_types = [t for t in topic_types if t in self.analyzer.target_msg_types]
                        if filtered_types:
