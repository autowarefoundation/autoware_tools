...
{
  return calculate_epdms_ && enabled_metrics_.synthetic_epdms &&
  ...
  rosbag2_cpp::Writer * bag_writer)
{
  ...
  std::vector<metrics::EpdmsMetricSnapshot> human_snapshots(calculate_epdms_ ? total_samples : 0U);
    ...
    while (true) {
      ...
      try {
        ...
        if (calculate_epdms_) {
          ...
          human_snapshots[i] = calculate_human_reference_snapshot(
  ...
  if (!calculate_epdms_) {
    RCLCPP_INFO(logger_, "Skipping EPDMS phase: open_loop.calculate_epdms is false");
    RCLCPP_INFO(logger_, "Sequential output phase started: bag writing and summary aggregation");
    for (std::size_t i = 0; i < total_samples; ++i) {
      if (bag_writer) {
        save_metrics_to_bag(
          metrics_list_[i], metrics::SyntheticEpdmsMetrics{}, evaluation_data_list[i], *bag_writer);
      }
    }
    calculate_summary();
    RCLCPP_INFO(
  ...
  const EvaluationData & eval_data, rosbag2_cpp::Writer & bag_writer)
{
  ...
  write_array(
    calculate_epdms_ && enabled_metrics_.time_to_collision_within_bound, metrics.ttc,
    metric_topic("ttc"));
...
std::vector<std::pair<std::string, std::string>> OpenLoopEvaluator::get_result_topics() const
{
  ...
  if (calculate_epdms_) {
    ...
    add_epdms_topic_if(
      enabled_metrics_.extended_comfort, "extended_comfort", "std_msgs/msg/Float64");
    ...
    add_epdms_topic_if(
      enabled_metrics_.no_at_fault_collision, "time_to_at_fault_collision_s",
      "std_msgs/msg/Float64");
    add_epdms_availability_reason_if(
      enabled_metrics_.no_at_fault_collision, "no_at_fault_collision");
    add_epdms_topic_if(
    ...
    add_epdms_topic_if(
      add_synthetic_epdms, "synthetic_epdms_human_filtered", "std_msgs/msg/Float64");
    add_epdms_topic_if(
