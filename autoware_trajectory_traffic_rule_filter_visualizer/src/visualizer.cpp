...
struct Control {
  ...
  std::string format_value() const {
    ...
    std::stringstream ss;
    if (is_int) ss << static_cast<int>(value);
...
void on_mouse(int event, int x, int y, int /*flags*/, void* userdata) {
  ...
  if (state->active_control != -1 && (event == cv::EVENT_MOUSEMOVE || event == cv::EVENT_LBUTTONDOWN)) {
...
void draw_plot(AppState & state)
{
  ...
  for (auto& c : state.controls) {
    ...
    cv::putText(plot, c.name + ": " + c.format_value(), cv::Point(c.rect.x, c.rect.y - 5), 
                cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(200, 200, 200), 1);
    
    if (c.is_bool) {
      cv::rectangle(plot, cv::Rect(c.rect.x, c.rect.y, 20, 20), cv::Scalar(100, 100, 100), 1);
      if (c.value > 0.5) {
        cv::rectangle(plot, cv::Rect(c.rect.x + 3, c.rect.y + 3, 14, 14), cv::Scalar(0, 255, 0), cv::FILLED);
      }
      cy += 45;
    } else {
      cv::rectangle(plot, c.rect, cv::Scalar(60, 60, 60), cv::FILLED);
      int handle_x = c.rect.x + static_cast<int>((c.value - c.min_val) / (c.max_val - c.min_val) * c.rect.width);
      cv::rectangle(plot, cv::Rect(handle_x - 3, c.rect.y - 2, 6, c.rect.height + 4), cv::Scalar(180, 180, 180), cv::FILLED);
      cy += 40;
    }
  }
  ...
  for (int y = 0; y < plot_height; y += res) {
    for (int x = 0; x < plot_width; x += res) {
      ...
      cv::rectangle(plot, 
                    cv::Point(x + plot_area_start_x, y + margin_top), 
                    cv::Point(x + plot_area_start_x + block_w, y + margin_top + block_h), 
                    color, cv::FILLED);
    }
  }
  ...
  for (double v = 0; v <= max_v; v += (max_v > 50 ? 10 : 5)) {
    ...
    cv::putText(plot, std::to_string(static_cast<int>(v)), cv::Point(x - 10, origin.y + 20), 
                cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255), 1);
  }
  cv::putText(plot, "Velocity (m/s)", cv::Point(plot_area_start_x + plot_width / 2 - 50, origin.y + 45), 
  ...
  for (double d = 0; d <= max_d; d += (max_d > 100 ? 20 : 10)) {
    ...
    cv::putText(plot, std::to_string(static_cast<int>(d)), cv::Point(origin.x - 30, y + 5), 
                cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255), 1);
  }
  cv::putText(plot, "Distance (m)", cv::Point(plot_area_start_x - 40, margin_top - 15), 
              cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
  ...
  if (legend_x + 80 < total_width) {
    ...
    cv::putText(plot, "Pass", cv::Point(legend_x + 20, legend_y + 12), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255), 1);
    cv::rectangle(plot, cv::Rect(legend_x, legend_y + 25, 15, 15), cv::Scalar(0, 0, 200), cv::FILLED);
    cv::putText(plot, "Stop", cv::Point(legend_x + 20, legend_y + 37), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255), 1);
  }
